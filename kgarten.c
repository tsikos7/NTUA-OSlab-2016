...
struct kgarten_struct {
    ...
    pthread_cond_t teacher_may_exit;
    pthread_cond_t child_may_enter;
    int children_that_may_enter;
...
void child_enter(struct thread_info_struct *thr)
{
    ...
    //CHILD can NOT ENTER whenever it wants... only when there is space for more and mutex allows it(always??)
    
    while (thr->kg->children_that_may_enter == 0)   {
        pthread_cond_wait(&thr->kg->child_may_enter, &thr->kg->mutex);
    }

    ++(thr->kg->vc);
    --(thr->kg->children_that_may_enter);

    if ((thr->kg->vt-1)*thr->kg->ratio >= thr->kg->vc) pthread_cond_broadcast(&thr->kg->teacher_may_exit);
    if (thr->kg->children_that_may_enter > 0) pthread_cond_broadcast(&thr->kg->child_may_enter);
...
void child_exit(struct thread_info_struct *thr)
{
    ...
    //CHILD can EXIT whenever it wants... no need for conditional wait
...
void teacher_enter(struct thread_info_struct *thr)
{
    ...
    //TEACHER can ENTER whenever he wants... no need for conditional wait

    ++(thr->kg->vt);
    thr->kg->children_that_may_enter += thr->kg->ratio;
...
void teacher_exit(struct thread_info_struct *thr)
{
    ...
    //TEACHER can NOT EXIT whenever it wants... only when there is space for the availlable CHILDREN if he leaves and mutex allows it(always??)
    
    while ((thr->kg->vt-1)*thr->kg->ratio < thr->kg->vc)    {
        pthread_cond_wait(&thr->kg->teacher_may_exit, &thr->kg->mutex);
    }
